...
XX                                                                           XX
XX                    Register Requirements for MIPS64                       XX
XX                                                                           XX
XX  This encapsulates all the logic for setting register requirements for    XX
XX  the MIPS64 architecture.                                                 XX
XX                                                                           XX
XX                                                                           XX
XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
...
int LinearScan::BuildNode(GenTree* tree)
{
    ...
    switch (tree->OperGet())
    {
        ...
        //case GT_MOD:
        //case GT_UMOD:
        //    NYI_IF(varTypeIsFloating(tree->TypeGet()), "FP Remainder in MIPS64");
        //    assert(!"Shouldn't see an integer typed GT_MOD node in MIPS64");
        //    srcCount = 0;
        //    break;
        ...
        case GT_CMPXCHG:
        {
            ...
            {
                // For MIPS exclusives requires a single internal register
                buildInternalIntRegisterDefForNode(tree);
            }

            // For MIPS exclusives the lifetime of the addr and data must be extended because
            // it may be used multiple during retries

            // For MIPS atomic cas the lifetime of the addr and data must be extended to prevent
            ...
            if (!cmpXchgNode->gtOpComparand->isContained())
            {
                ...
                // For MIPS exclusives the lifetime of the comparand must be extended because
        ...
        case GT_LEA:
        {
            ...
            // On MIPS64 we may need a single internal register
            // (when both conditions are true then we still only need a single internal register)
            if ((index != nullptr) && (cns != 0))
            {
                // MIPS64 does not support both Index and offset so we need an internal register
                buildInternalIntRegisterDefForNode(tree);
            }
        ...
        case GT_NULLCHECK:
            // MIPS64 implements NULLCHECK as a load to REG_R0, so no internal register
...
assert(!"unimplemented on MIPS yet");
    ...
    switch (simdTree->gtSIMDIntrinsicID)
    {
        ...
        case SIMDIntrinsicSelect:
            // TODO-MIPS64-CQ Allow lowering to see SIMDIntrinsicSelect so we can generate BSL VC, VA, VB
...
int LinearScan::BuildIndir(GenTreeIndir* indirTree)
{
    ...
    if (addr->isContained())
    {
        ...
        // On MIPS we may need a single internal register
        // (when both conditions are true then we still only need a single internal register)
        if ((index != nullptr) && (cns != 0))
        {
            // MIPS does not support both Index and offset so we need an internal register
...
int LinearScan::BuildCall(GenTreeCall* call)
{
    ...
    if (ctrlExpr != nullptr)
    {
        ...
        {
            // Fast tail call - make sure that call target is always computed in T9(MIPS64)
    ...
    for (GenTree* list = call->gtCallLateArgs; list; list = list->MoveNext())
    {
        ...
        if (argNode->OperGet() == GT_FIELD_LIST)
        {
            ...
            for (GenTreeFieldList* entry = argNode->AsFieldList(); entry != nullptr; entry = entry->Rest())
            {
                ...
                // MIPS64 could use int register with next float register for struct.
...
int LinearScan::BuildPutArgStk(GenTreePutArgStk* argNode)
{
    ...
    if ((putArgChild->TypeGet() == TYP_STRUCT) || (putArgChild->OperGet() == GT_FIELD_LIST))
    {
        ...
        {
            // We can use a ldp/stp sequence so we need two internal registers for MIPS64; one for ARM.
    ...
    else
    {
        ...
        else
        {
            ...
            if (blkNode->gtBlkOpKind == GenTreeBlk::BlkOpKindUnroll)
            {
                ...
                if (size >= 2 * REGSIZE_BYTES)
                {
                    // We will use ld/sd to reduce code size and improve performance
                    // so we need to reserve an extra internal register
                    buildInternalIntRegisterDefForNode(blkNode);
                }
            }
