...
XX                                                                           XX
XX             Lowering for MIPS64 common code                               XX
XX                                                                           XX
XX  This encapsulates common logic for lowering trees for the MIPS64         XX
...
#ifdef _TARGET_MIPS64_ // This file is ONLY used for MIPS64 architectures
...
assert(!"unimplemented on MIPS yet");
    ...
    //
    // MIPS64 does not support most forms of compare zero for Unsigned values
    ...
    {
        auto setAllVector = node->gtSIMDSize > 8 ? NI_MIPS64_SIMD_SetAllVector128 : NI_MIPS64_SIMD_SetAllVector64;

        auto origOp1 = node->gtOp.gtOp1;

        switch (intrinsicID)
        {
            case NI_MIPS64_SIMD_GT_ZERO:
                // Unsigned > 0 ==> !(Unsigned == 0)
                node->gtOp.gtOp1 =
                    comp->gtNewSimdHWIntrinsicNode(node->TypeGet(), node->gtOp.gtOp1, NI_MIPS64_SIMD_EQ_ZERO,
                                                   node->gtSIMDBaseType, node->gtSIMDSize);
                node->gtHWIntrinsicId = NI_MIPS64_SIMD_BitwiseNot;
                BlockRange().InsertBefore(node, node->gtOp.gtOp1);
                break;
            case NI_MIPS64_SIMD_LE_ZERO:
                // Unsigned <= 0 ==> Unsigned == 0
                node->gtHWIntrinsicId = NI_MIPS64_SIMD_EQ_ZERO;
                break;
            case NI_MIPS64_SIMD_GE_ZERO:
            case NI_MIPS64_SIMD_LT_ZERO:
                // Unsigned >= 0 ==> Always true
                // Unsigned < 0 ==> Always false
                node->gtHWIntrinsicId = setAllVector;
                node->gtOp.gtOp1      = comp->gtNewLconNode((intrinsicID == NI_MIPS64_SIMD_GE_ZERO) ? ~0ULL : 0ULL);
                BlockRange().InsertBefore(node, node->gtOp.gtOp1);
...
{
    // There are no contained operands for MIPS.
}
...
void Lowering::ContainCheckIndir(GenTreeIndir* indirNode)
{
    ...
    //
    // TODO-MIPS64-CQ: handle other addr mode expressions that could be marked
    // as contained.
...
assert(!"unimplemented on MIPS yet");
    ...
    switch (simdNode->gtSIMDIntrinsicID)
    {
        ...
        case SIMDIntrinsicOpInEquality:
            // TODO-MIPS64-CQ Support containing 0
            break;
    ...
    switch (HWIntrinsicInfo::lookup(node->gtHWIntrinsicId).form)
    {
        ...
        case HWIntrinsicInfo::SimdInsertOp:
            if (op2->IsCnsIntOrI())
            {
                ...
                if (op3->OperIs(GT_HWIntrinsic) && (op3->AsHWIntrinsic()->gtHWIntrinsicId == NI_MIPS64_SIMD_GetItem))
                {
