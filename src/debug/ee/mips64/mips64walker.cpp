...
//*****************************************************************************
// File: Mips64walker.cpp
//

//
// MIPS64 instruction decoding/stepping logic
//
//*****************************************************************************
...
{
    _ASSERTE(!"=====Not implements for mips64. -1");
    opcode = opcode >> 5;
...
{
    _ASSERTE(!"=====Not implements for mips64. -2");
    ...
    LOG((LF_CORDB, LL_INFO100000, "Mips64Walker::Decode instruction at %p, opcode: %x\n", m_ip,opcode));
...
{
    _ASSERTE(!"=====Not implements for mips64. -3");
    ...
    */
    ...
    if ((opcode & 0x1F000000) == 0x10000000)  //ADR & ADRP
    {
        ...
        {
            offset = offset << 12;
            LOG((LF_CORDB, LL_INFO100000, "Mips64Walker::Simulate opcode: %x to ADRP X%d %p\n", opcode, RegNum, offset));
        }
        else
        {
            LOG((LF_CORDB, LL_INFO100000, "Mips64Walker::Simulate opcode: %x to ADR X%d %p\n", opcode, RegNum, offset));
        }
    ...
    else if ((opcode & 0x3B000000) == 0x18000000) //LDR Literal (General or SIMD)
     {
        ...
        RegNum = (opcode & 0x1F);
        LOG((LF_CORDB, LL_INFO100000, "Mips64Walker::Simulate opcode: %x to LDR[SW] | PRFM X%d %p\n", opcode, RegNum, offset));
    }
    ...
    if (walk == WALK_BRANCH || walk == WALK_CALL || walk == WALK_RETURN)
    {
        ...
        LOG((LF_CORDB, LL_INFO100000, "Mips64Walker::Simulate opcode: %x  is a Control Flow instr \n", opcode));

        if (walk == WALK_CALL) //initialize Lr
        {
            SetRA(context, (PCODE)address + MAX_INSTRUCTION_LENGTH);
            LOG((LF_CORDB, LL_INFO100000, "Mips64Walker::Simulate opcode: %x  is a Call instr, setting RA to %p \n", opcode,GetRA(context)));
        }
    }
    else if(RegNum >= 0)
    {
        ...
        if ((opcode & 0x3B000000) == 0x18000000) //LDR Literal
        {
            ...
            if ((opcode & 0x4000000)) //LDR literal for SIMD
            {
                ...
                LOG((LF_CORDB, LL_INFO100000, "Mips64Walker::Simulate opcode: %x to LDR V%d %p\n", opcode, RegNum, offset));
                short opc = (opcode >> 30);
                switch (opc)
                {
                ...
                case 2: //SIMD 16 byte data
                    ...
                    break;
                default:
                    LOG((LF_CORDB, LL_INFO100000, "Mips64Walker::Simulate Unknown opcode: %x [LDR(litera,SIMD &FP)]  \n", opcode));
                    _ASSERTE(!("Mips64Walker::Simulated Unknown opcode"));
            ...
            else
            {
                ...
                case 0: //4byte data into Wt
                    LOG((LF_CORDB, LL_INFO100000, "Mips64Walker::Simulate opcode: %x to LDR W%d %p\n", opcode, RegNum, offset));
                    RegContents = 0xFFFFFFFF & RegContents;  //zero the upper 32bits
                    SetReg(context, RegNum, RegContents);
                    break;

                case 1: //8byte data into Xt
                    LOG((LF_CORDB, LL_INFO100000, "Mips64Walker::Simulate opcode: %x to LDR X%d %p\n", opcode, RegNum, offset));
                    SetReg(context, RegNum, RegContents);
                    break;

                case 2: //LDRSW
                    LOG((LF_CORDB, LL_INFO100000, "Mips64Walker::Simulate opcode: %x to LDRSW X%d %p\n", opcode, RegNum, offset));
                    ...
                    break;
                case 3:
                    LOG((LF_CORDB, LL_INFO100000, "Mips64Walker::Simulate opcode: %x  as PRFM ,but do nothing \n", opcode));

                    break;
                default:
                    LOG((LF_CORDB, LL_INFO100000, "Mips64Walker::Simulate Unknown opcode: %x [LDR(literal)]  \n", opcode));
                    _ASSERTE(!("Mips64Walker::Simulated Unknown opcode"));
        ...
        LOG((LF_CORDB, LL_INFO100000, "Mips64Walker::Simulate opcode: %x  to update Reg X[V]%d, as %p \n", opcode, RegNum, GetReg(context, RegNum)));
    }
    //else  Just execute the opcodes as IS
    //{
    //}
...
{
    _ASSERTE(!"=====Not implements for mips64. -4");
#ifdef _DEBUG
    ...
    if ((opcode & 0x7C000000) == 0x14000000) // Decode B & BL
    {
        ...
        {
            walk = WALK_CALL;
            LOG((LF_CORDB, LL_INFO100000, "Mips64Walker::Decoded opcode: %x to BL %p \n", opcode, offset));
        }
        else
        {
            walk = WALK_BRANCH; //B
            LOG((LF_CORDB, LL_INFO100000, "Mips64Walker::Decoded opcode: %x to B %p \n", opcode, offset));
        }
        return TRUE;
    }
    ...
    if ((opcode & 0xFF000010) == 0x54000000) // B.cond
    {
        ...
        if (result)
        {
            walk = WALK_BRANCH;
            offset = Expand19bitoffset(opcode);
            LOG((LF_CORDB, LL_INFO100000, "Mips64Walker::Decoded opcode: %x to B.cond %p \n", opcode, offset));
        }
        else // NOP
        {
            walk = WALK_UNKNOWN;
            LOG((LF_CORDB, LL_INFO100000, "Mips64Walker::Decoded opcode: %x to B.cond but evaluated as NOP \n", opcode));
    ...
    if ((opcode & 0xFE000000) == 0x34000000) // CBNZ || CBZ
    {
        ...
        {
            result = RegContent != 0;
            LOG((LF_CORDB, LL_INFO100000, "Mips64Walker::Decoded opcode: %x to CBNZ X%d \n", opcode, RegNum));
        }
        else //CBZ
        {
            result = RegContent == 0;
            LOG((LF_CORDB, LL_INFO100000, "Mips64Walker::Decoded opcode: %x to CBZ X%d \n", opcode, RegNum));
        }
        ...
        if (result)
        {
            walk = WALK_BRANCH;
            offset = Expand19bitoffset(opcode);
            LOG((LF_CORDB, LL_INFO100000, "Mips64Walker::Decoded opcode: %x to CB[N]Z X%d %p \n", opcode, RegNum, offset));
        }
        else // NOP
        {
            walk = WALK_UNKNOWN;
    ...
    if ((opcode & 0x7E000000) == 0x36000000)    // TBNZ || TBZ
    {
        ...
        if (opcode & 0x01000000) //TBNZ
        {
            ...
            LOG((LF_CORDB, LL_INFO100000, "Mips64Walker::Decoded opcode: %x to TBNZ X%d \n", opcode, RegNum));
        }
        else //TBZ
        {
            result = (RegContent & bit_val) == 0;
            LOG((LF_CORDB, LL_INFO100000, "Mips64Walker::Decoded opcode: %x to CB[N]Z X%d \n", opcode, RegNum));
        }
        if (result)
        {
            walk = WALK_BRANCH;
            ...
            LOG((LF_CORDB, LL_INFO100000, "Mips64Walker::Decoded opcode: %x to TB[N]Z X%d %p \n", opcode, RegNum, offset));
        }
        else // NOP
        {
            walk = WALK_UNKNOWN;
...
BOOL  NativeWalker::DecodeCallInst(const PRD_TYPE& opcode, int& RegNum, WALK_TYPE& walk)
{
    if ((opcode & 0xFF9FFC1F) == 0xD61F0000) // BR, BLR or RET -Unconditional Branch (register) instructions
    {
        ...
        switch (op)
        {
        case 0: LOG((LF_CORDB, LL_INFO100000, "Mips64Walker::Decoded opcode: %x to BR X%d\n", opcode, RegNum));
            walk = WALK_BRANCH;
            break;
        case 1: LOG((LF_CORDB, LL_INFO100000, "Mips64Walker::Decoded opcode: %x to BLR X%d\n", opcode, RegNum));
            walk = WALK_CALL;
            break;
        case 2: LOG((LF_CORDB, LL_INFO100000, "Mips64Walker::Decoded opcode: %x to Ret X%d\n", opcode, RegNum));
            walk = WALK_RETURN;
            break;
        default:
            LOG((LF_CORDB, LL_INFO100000, "Mips64Walker::Simulate Unknown opcode: %x [Branch]  \n", opcode));
            _ASSERTE(!("Mips64Walker::Decoded Unknown opcode"));
        }
